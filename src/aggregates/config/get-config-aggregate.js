import { createConfigAggregate } from './create-config-aggregate'
...
export function getConfigAggregate(eventId, client, options = {}) {
  ...
  const aggregate = createConfigAggregate(eventId, client, options)
  cache.set(key, {
    aggregate,
    client,
    createdAt: Date.now(),
...
export function clearConfigAggregate(eventId) {
  if (!eventId) {
    cache.clear()
    return
  }
  cache.delete(buildKey(eventId))
}
