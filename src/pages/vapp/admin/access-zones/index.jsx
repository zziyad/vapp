...
export default function AccessZonesPage() {
  ...
  useEffect(() => {
    ...
    return aggregate.accessZone.subscribe((state) => {
      setItems(state.list || [])
  ...
  const fetchItems = useCallback(async () => {
    if (!aggregate?.accessZone || !eventId) return
    try {
  ...
  const handleSave = async () => {
    if (!validateForm() || !aggregate?.accessZone || !eventId) return
    ...
    try {
      ...
      setFormErrors({ submit: err.message || 'Failed to save access zone' })
    } finally {
      setSaving(false)
    }
  }
  ...
  const handleDelete = async (item, hardDelete = false) => {
    ...
    try {
      await aggregate.accessZone.delete(item.id, hardDelete)
    } catch (err) {
      console.error('Failed to delete access zone:', err)
      alert(err.message || 'Failed to delete access zone')
    }
  }
  ...
  return (
    <Container className="py-6">
      <VappPageHeader
        eventId={eventId}
        pageTitle="Access Zones"
        pageDescription="Manage venue access zones for this event"
      />
      ...
      <div className="grid grid-cols-1 lg:grid-cols-4 gap-6">
        ...
        <div className="lg:col-span-3">
          <PermissionGuard permission={PERMISSIONS.VAPP.CONFIG.ACCESS_ZONE.READ}>
            <div className="space-y-6">
              <div className="flex items-center justify-between">
                <div>
                  <h3 className="text-lg font-semibold">Access Zones</h3>
                  <p className="text-sm text-gray-600">
                    Define venue access zones (e.g., Infinity, STA, PAF)
                  </p>
                </div>
                <PermissionGuard permission={PERMISSIONS.VAPP.CONFIG.ACCESS_ZONE.WRITE}>
              ...
              {loading && (
                <div className="text-center py-12">
                  ...
                  <p className="text-sm text-gray-600 mt-2">Loading access zones...</p>
                </div>
              )}
              ...
              {!loading && items.length === 0 && (
                <Card>
                  <CardContent className="py-12 text-center text-gray-500">
                    No access zones found. Create your first access zone to get started.
                  </CardContent>
                </Card>
              )}
              ...
              <Dialog open={showDialog} onOpenChange={setShowDialog}>
                <DialogContent>
                  <DialogHeader>
                    <DialogTitle>
                      {editingItem ? 'Edit Access Zone' : 'Create Access Zone'}
                    </DialogTitle>
                    <DialogDescription>
                      {editingItem
                        ? 'Update the access zone information below.'
                        : 'Enter the access zone details below.'}
                    </DialogDescription>
                  </DialogHeader>
