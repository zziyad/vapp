...
export default function AccessTypesPage() {
  ...
  useEffect(() => {
    ...
    return aggregate.accessType.subscribe((state) => {
      setItems(state.list || [])
  ...
  const fetchItems = useCallback(async () => {
    if (!aggregate?.accessType || !eventId) return
    try {
  ...
  const handleSave = async () => {
    if (!validateForm() || !aggregate?.accessType || !eventId) return
    ...
    try {
      ...
      setFormErrors({ submit: err.message || 'Failed to save access type' })
    } finally {
      setSaving(false)
    }
  }
  ...
  const handleDelete = async (item, hardDelete = false) => {
    if (!aggregate?.accessType) return
    if (item.is_global) {
      alert('Global access types cannot be deleted')
      return
    }
    if (!confirm(
      hardDelete
    ...
    try {
      await aggregate.accessType.delete(item.id, hardDelete)
    } catch (err) {
      console.error('Failed to delete access type:', err)
      alert(err.message || 'Failed to delete access type')
    }
  }
  ...
  return (
    <Container className="py-6">
      <VappPageHeader
        eventId={eventId}
        pageTitle="Access Types"
        pageDescription="Manage access types (global defaults + event-specific custom)"
      />
      ...
      <div className="grid grid-cols-1 lg:grid-cols-4 gap-6">
        ...
        <div className="lg:col-span-3">
          <PermissionGuard permission={PERMISSIONS.VAPP.CONFIG.ACCESS_TYPE.READ}>
            <div className="space-y-6">
              <div className="flex items-center justify-between">
                ...
                <PermissionGuard permission={PERMISSIONS.VAPP.CONFIG.ACCESS_TYPE.WRITE}>
              ...
              {loading && (
                <div className="text-center py-12">
                  ...
                  <p className="text-sm text-gray-600 mt-2">Loading access types...</p>
                </div>
              )}
              ...
              <Dialog open={showDialog} onOpenChange={setShowDialog}>
                <DialogContent>
                  <DialogHeader>
                    <DialogTitle>
                      {editingItem ? 'Edit Access Type' : 'Create Access Type'}
                    </DialogTitle>
                    <DialogDescription>
                      {editingItem
                        ? 'Update the access type information below.'
                        : 'Enter the access type details below.'}
                    </DialogDescription>
                  </DialogHeader>
                  <div className="space-y-4 py-4">
                    ...
                    <div>
                      ...
                      <Input
                        ...
                        placeholder="Access Only (no Parking)"
