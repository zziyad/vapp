...
export default function ValidityPage() {
  ...
  useEffect(() => {
    ...
    return aggregate.validity.subscribe((state) => {
      setItems(state.list || [])
  ...
  const fetchItems = useCallback(async () => {
    if (!aggregate?.validity || !eventId) return
    try {
  ...
  const handleSave = async () => {
    if (!validateForm() || !aggregate?.validity || !eventId) return
    ...
    try {
      ...
      console.error('Failed to save validity:', err)
      setFormErrors({ submit: err.message || 'Failed to save validity' })
    } finally {
      setSaving(false)
    }
  }
  ...
  const handleDelete = async (item, hardDelete = false) => {
    ...
    try {
      await aggregate.validity.delete(item.id, hardDelete)
    } catch (err) {
      console.error('Failed to delete validity:', err)
      alert(err.message || 'Failed to delete validity')
    }
  }
  ...
  return (
    <Container className="py-6">
      <VappPageHeader
        eventId={eventId}
        pageTitle="Validity"
        pageDescription="Manage validity periods for this event"
      />
      ...
      <div className="grid grid-cols-1 lg:grid-cols-4 gap-6">
        ...
        <div className="lg:col-span-3">
          <PermissionGuard permission={PERMISSIONS.VAPP.CONFIG.VALIDITY.READ}>
            <div className="space-y-6">
              <div className="flex items-center justify-between">
                <div>
                  <h3 className="text-lg font-semibold">Validity Periods</h3>
                  <p className="text-sm text-gray-600">
                    Define validity periods (e.g., ALL, NMD, M1-M18 for match days)
                  </p>
                </div>
                <PermissionGuard permission={PERMISSIONS.VAPP.CONFIG.VALIDITY.WRITE}>
              ...
              {loading && (
                <div className="text-center py-12">
                  ...
                  <p className="text-sm text-gray-600 mt-2">Loading validity periods...</p>
                </div>
              )}
              ...
              {!loading && items.length === 0 && (
                <Card>
                  <CardContent className="py-12 text-center text-gray-500">
                    No validity periods found. Create your first validity period to get started.
                  </CardContent>
                </Card>
              )}
              ...
              <Dialog open={showDialog} onOpenChange={setShowDialog}>
                <DialogContent>
                  <DialogHeader>
                    <DialogTitle>
                      {editingItem ? 'Edit Validity' : 'Create Validity'}
                    </DialogTitle>
                    <DialogDescription>
                      {editingItem
                        ? 'Update the validity period information below.'
                        : 'Enter the validity period details below.'}
                    </DialogDescription>
                  </DialogHeader>
